...
function duckShoot(ammo, aim, ducks) {
  //dibulatkan ke bawah
  const ducksHitPerShot = Math.floor(aim * ammo);
  

  // Buat array dari string bebek agar lebih mudah dimanipulasi
  const duckArray = ducks.split("");

  // Ulangi susunan bebek dan catat jumlah bidikan yang diambil
  let shotsTaken = 0;
  for (let i = 0; i < duckArray.length; i++) {
    if (duckArray[i] === "2") {
      // Periksa apakah ada bebek
      if (shotsTaken < ducksHitPerShot) {
        // Periksa apakah ammo cukup
        duckArray[i] = "X"; // Tandai bebek itu sebagai tembakan
        shotsTaken++;
      } 
    }
  }

  // Gabungkan kembali array bebek yang telah dimodifikasi menjadi string dan kembalikan
  return duckArray.join("");
}
